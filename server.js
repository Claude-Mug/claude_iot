...
app.use(express.static(__dirname)); // Sert commandes.html, CSS, JS, etc.
...
// === 🔹 Mémoire tampon pour messages ESP (non stockés en BD) ===
let messages = [];
...
app.post("/set_command", async (req, res) => {
  ...
  try {
    ...
    console.log(`✅ Nouvelle commande enregistrée : ${device} -> ${action}`);
...
// === 🔹 Dernière commande (JSON complet pour l'interface web) ===
...
// === 🔹 Dernière commande (texte brut pour microcontrôleur ESP) ===
app.get("/last_command", async (req, res) => {
  try {
    const { rows } = await pool.query("SELECT device, action FROM commandes ORDER BY id DESC LIMIT 1");
    if (rows.length === 0) return res.send("aucune_commande");
